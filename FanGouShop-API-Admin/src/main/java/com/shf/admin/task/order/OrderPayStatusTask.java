...
import com.shf.service.service.AlipayTaskService;
...
public class OrderPayStatusTask {
    ...
    @Autowired
    private AlipayTaskService alipayTaskService;

    @Scheduled(fixedDelay = 1000 * 60L) //1分钟同步一次数据
    public void init() {
        logger.info("---OrderPayStatusTask task------produce Data with fixed rate task: Execution Time - {}", DateUtil.nowDateTime());
        try {
            alipayTaskService.queryAliPayResult();
        } catch (Exception e) {
            e.printStackTrace();
